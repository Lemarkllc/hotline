...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (res.status === 401 && !skipAuth) {
    ...
    // Повтор ТОЛЬКО если рефреш реально удался — если запрос после этого снова 401,
    // это НЕ обязательно "сессия истекла": например, /auth/change-password отдаёт
    // 401 на неверный ТЕКУЩИЙ пароль (обычная бизнес-ошибка, а не протухший токен) —
    // раньше это молча разлогинивало пользователя вместо показа "неверный пароль"
    // (баг: пропадал прямо на экране смены пароля, оставляя старый пароль рабочим).
    // Блокировку/удаление аккаунта посреди сессии уже обрабатывает сам tryRefresh()
    // (authService.refresh проверяет user.status и роняет !res.ok до этого места).
